...
class TestBlock(unittest.TestCase):
    ...
    def test_success(self):
        ...
        message_for_c =  (ctypes.c_ubyte * len(message))(*message)
        key_for_c = (ctypes.c_ubyte * len(key))(*key)

        ciphertext = aes.aes.AES(key).encrypt_block(message)
        c_ciphertext = c_aes.aes_encrypt_block(message_for_c, key_for_c)
        # convert the necrypted c_cipher text to bytes
        c_ciphertext = bytes(c_ciphertext[:16])

        self.assertEqual(self.aes.decrypt_block(ciphertext), message)
        self.assertEqual(ciphertext, c_ciphertext)
